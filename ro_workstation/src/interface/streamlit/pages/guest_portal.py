...
def render() -> None:
    ...
    guest_tabs = st.tabs(["📊 Business Overview", "👥 Organizational Chart"])
    ...
    with guest_tabs[0]:
        ...
        if not units_df.empty:
            ...
            active_units_list = [u for u in master_service.repo.get_by_category("UNIT") if str(u.code) in active_sols]
            
            branch_head_grades = []
            for u in active_branches.to_dict(orient="records"):
                u_code = str(u["Code"])
                u_record = next((ur for ur in active_units_list if str(ur.code) == u_code), None)
                if u_record:
